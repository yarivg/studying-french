...
nothing: every lesson, the stylesheet, the scripts and all 1,493 vocabulary
entries are inlined, so the file works from a phone's Files app, a USB stick,
or an email attachment, with no server and no network.

Run:  python3 tools/build-single-file.py
Out:  le-carnet-offline.html
"""
...
OUT = os.path.join(ROOT, "le-carnet-offline.html")
...
def main():
    ...
    html = html.replace(
        "<title>Le Carnet — a French course</title>",
        "<title>Le Carnet — a French course (offline)</title>")
